...
class SentimentAnalysis:
    ...
    def __do(self,combination:dict,gs=True):
        ...
        if gs:
            if results['test_accuracy'] > self.best_accuracy:
                ...
                self.best_loss = results['test_loss']
                self.best_params_dict = combination
                self.best_predictions = results['predictions']
                print(f"Best combination so far: {combination}\nAccuracy:{self.best_accuracy}") 
        else: 
            return results
    ...
    def run(self,combination_dict,model=None):
        ...
        self.metrics(results['predictions'],self.y_test)
        return results
    ...
    def grid_search(self,param_dict,default_params_dict):
        ...
        self.metrics(self.best_predictions,self.y_test)
        return self.best_params_dict

    def metrics(self,y_pred,y_test):
        print(f"Accuracy: {accuracy_score(y_true=y_test,y_pred=y_pred)}")
        print(f"Recall: {recall_score(y_true=y_test,y_pred=y_pred,average='macro')}")
        print(f"Precision: {precision_score(y_true=y_test,y_pred=y_pred,average='macro')}")
